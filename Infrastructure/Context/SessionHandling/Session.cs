...
public class Session(IServiceProvider serviceProvider)
{
    ...
    public async Task MoveStateAsync<T>(VeniInteractionContext context) where T : ISessionState
    {
        ...
        var newState = ActivatorUtilities.CreateInstance<T>(serviceProvider);
...
#region Asyncronous Component Interactions
    private TaskCompletionSource<SocketMessageComponent> _asyncComponentInteraction;

    public Task<SocketMessageComponent> AwaitComponentResponseAsync()
    {
        this._asyncComponentInteraction = new TaskCompletionSource<SocketMessageComponent>();
        return this._asyncComponentInteraction.Task;
    }

    public async Task<bool> HandleComponentResponseAsync(SocketMessageComponent component)
    {
        if (this._asyncComponentInteraction is null || this._asyncComponentInteraction.Task.IsCompleted)
            return false;
        await Task.Run(() => this._asyncComponentInteraction.SetResult(component));
        return true;
    }
#endregion

#region Asyncronous Component Interactions
    private TaskCompletionSource<SocketModal> _asyncModalInteraction;

    public Task<SocketModal> AwaitModalResponseAsync()
    {
        this._asyncModalInteraction = new TaskCompletionSource<SocketModal>();
        return this._asyncModalInteraction.Task;
    }

    public async Task<bool> HandleModalResponseAsync(SocketModal component)
    {
        if (this._asyncModalInteraction is null || this._asyncModalInteraction.Task.IsCompleted)
            return false;
        await Task.Run(() => this._asyncModalInteraction.SetResult(component));
        return true;
    }
#endregion
