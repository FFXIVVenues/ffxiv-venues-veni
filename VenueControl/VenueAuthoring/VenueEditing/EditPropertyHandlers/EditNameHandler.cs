...
public class EditNameHandler : IComponentHandler
{
    ...
    public EditNameHandler(IAuthorizer authorizer, IApiService apiService)
    {
    ...
    public async Task HandleAsync(MessageComponentVeniInteractionContext context, string[] args)
    {
        ...
        if (!this._authorizer.Authorize(user, Permission.EditVenue, venue).Authorized)
        {
            await context.Interaction.FollowupAsync(
                "Aaaah. You'll need to speak to my owners at FFXIV Venues to change the name of your venue. 🥲");
            return;
        }
