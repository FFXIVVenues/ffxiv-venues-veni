...
class BannerEntrySessionState(IAuthorizer authorizer, HttpClient httpClient, VenueAuthoringContext authoringContext) : ISessionState<VenueAuthoringContext>
{
    ...
    public Task EnterState(VeniInteractionContext interactionContext)
    {
        ...
        if (!authorizer.Authorize(interactionContext.Interaction.User.Id, Permission.EditPhotography, venue).Authorized)
            return interactionContext.MoveSessionToStateAsync<ManagerEntrySessionState, VenueAuthoringContext>(authoringContext);

        interactionContext.RegisterMessageHandler(this.OnMessageReceived);
        interactionContext.SetBackClearanceAmount(3);
        return interactionContext.Interaction.RespondAsync("What **banner image** would you like to use?\nBanners are 600x300; I'll do the scaling/cropping for you :heart:.",
            new ComponentBuilder()
                .WithBackButton(interactionContext)
                .WithSkipButton<ManagerEntrySessionState, ConfirmVenueSessionState>(interactionContext, authoringContext)
                .Build());
    }
    ...
    private async Task OnMessageReceived(MessageVeniInteractionContext c)
    {
        ...
        component.WithButton("Looks good!", c.RegisterComponentHandler(async cm =>
        {
            cm.Session.SetItem(SessionKeys.BANNER_URL, cm.Interaction.Message.Attachments.First().ProxyUrl);
            if (cm.Session.InEditing())
                await cm.MoveSessionToStateAsync<ConfirmVenueSessionState, VenueAuthoringContext>(authoringContext);
            else
                await cm.MoveSessionToStateAsync<ManagerEntrySessionState, VenueAuthoringContext>(authoringContext);
        }, ComponentPersistence.ClearRow));
        component.WithButton("Let's try another!", c.RegisterComponentHandler(cm => cm.Interaction.FollowupAsync("Alrighty, send over another image! :heart:"), ComponentPersistence.ClearRow), ButtonStyle.Secondary);
